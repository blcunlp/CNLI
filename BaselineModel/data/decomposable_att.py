...
class MyModel(object):
  ...
  def input_encoding(self):
    ...
    with tf.variable_scope("encode_x"):
      self.x_output=self.two_layer_dense(self.input_xemb,self.config.hidden_units,
                                        scope="x_fnn",regularizer=l2_regularizer(self.config.l2_strength) )
    ...
    with tf.variable_scope("encode_y"):
      self.y_output=self.two_layer_dense(self.input_yemb,self.config.hidden_units,
                                         scope="y_fnn",regularizer=l2_regularizer(self.config.l2_strength))
  ...
  def compare(self):
    ...
    with tf.variable_scope("compare"):
      with tf.variable_scope("compare-xy"):
        co_xy = tf.concat([self.x_output,self.weighted_y],axis=-1) 
        v_co_xy=self.two_layer_dense(co_xy,self.config.hidden_units,
                                     scope="compare_xy",regularizer=l2_regularizer(self.config.l2_strength))
        self.v_co_xy=v_co_xy*self.x_mask[:,:,None]

        if self.is_training and self.config.keep_prob < 1:
          self.v_co_xy = tf.nn.dropout(self.v_co_xy,self.config.keep_prob)  

      with tf.variable_scope("compare-yx"):
        co_yx = tf.concat([self.y_output,self.weighted_x],axis=-1) 
        v_co_yx=self.two_layer_dense(co_yx,self.config.hidden_units,
                                     scope="compare_yx",regularizer=l2_regularizer(self.config.l2_strength))
  ...
  def aggregate(self):
    ...
    with tf.variable_scope("pooling"):
      v1=tf.reduce_sum(self.v_co_xy,axis=1)
      v2=tf.reduce_sum(self.v_co_yx,axis=1)
